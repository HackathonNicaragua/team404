<!--Footer-->
<footer class="page-footer stylish-color-dark">

    <!--Footer Links-->
    <div class="container">

        <!-- Footer links -->
        <div class="row">

            <!--First column-->
            <div class="col-md-12">
                <img src="img/logo/Logo blanco.png" alt="">
            </div>
            <!--/.First column-->
            <div class="col-md-10 col-md-offset-1">
                <div class="col-md-7">
                    <div class="col-md-6">
                        
                        <p><a href="#!">Quiénes somos</a></p>
                        <p><a href="#!">Preguntas Frecuentes</a></p>
                        <p><a href="#!">Contácto</a></p>
                        <p><a href="#!">Soporte Técnico</a></p>
                    </div>
                    <!--/.Second column-->

                    <!--Third column-->
                    <div class="col-md-6">
                        <p><a href="#!">Certificaciones</a></p>
                        <p><a href="#!">Seguridad</a></p>
                        <p><a href="#!">Nuestros socios</a></p>
                        <p><a href="#!">Guia para padres</a></p>
                    </div>
                
                </div>
                <div class="col-md-5">
                    <br>
                    <p class="color-white">
                        Obtenga información acerca de Educarte.ni
                    </p>
                    
                    <form class="col-md-12">
                        <div class="row">
                            <div class="input-field col-md-10">
                                <input id="email" type="email" class="validate">
                                <label for="email" data-error="wrong" data-success="right">Email</label>
                            </div>
                            <div class="col-md-2">
                                <button class="btn btn-env btn-rounded waves-effect waves-light"> Suscribirse</button>
                            </div>
                        </div>

                        <p>
                            <input type="checkbox" id="test6" checked="checked" />
                            <label for="test6" class="color-white">Estoy deacuerdo con las condiciones de uso</label>
                        </p>
                    </form>
                </div>
            </div>

           
        </div>
        <!-- Footer links -->

        <div class="row">
            <!--Grid column-->
            <div class="col-md-8 col-lg-9">
                <div class="col-md-6">
                    <p class="color-white">© 2017 Copyright: <a href=""><strong> Educarte.ni</strong></a></p>
                </div>
                <div class="col-md-6">
                    <a href="" class="enlacesfot">Condiciones de uso</a> 
                    <a href="" class="enlacesfot">Politicas de privacidad</a>
                </div>
                <!--/.Copyright-->
            </div>
            <!--Grid column-->

            <!--Grid column-->
            <div class="col-md-4 col-lg-3">

                <!--Social buttons-->
                <div class="text-center text-md-left">
                    <ul>
                        <a class="btn-floating btn-large"><i class="fa fa-facebook"> </i></a>
                     
                        <a class="btn-floating btn-large"><i class="fa fa-google-plus"> </i></a>
                        <a class="btn-floating btn-large btn-gm"><i class="fa fa-google-plus"> </i></a>
                    </ul>
                </div>
                <!--/.Social buttons-->

            </div>
            <!--Grid column-->

        </div>

    </div>

</footer>
<!--/.Footer-->